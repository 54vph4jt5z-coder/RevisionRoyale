...
import {
  ...
  Settings, Flame, Bell, Play, Star, Lock, Leaf, Calculator, Atom, FlaskConical,
  LogOut, Sun, Moon, Send, Copy, CheckCircle2, Target, Crown
} from "lucide-react";
import "./styles.css";
...
const COURSE_TOPICS = {
  Biology: {
    icon: <Leaf />,
    color: "green",
    "National 5": ["Cell Biology", "Multicellular Organisms", "Life On Earth", "Genetics", "Ecology"],
    Higher: ["DNA & Genome", "Metabolism & Survival", "Sustainability", "Physiology", "Genetic Engineering"],
    "Advanced Higher": ["Cells & Proteins", "Organisms & Evolution", "Investigative Biology", "Membranes", "Cell Signalling"]
  },
  Maths: {
    icon: <Calculator />,
    color: "blue",
    "National 5": ["Expressions", "Formulae", "Relationships", "Applications", "Numeracy"],
    Higher: ["Functions", "Straight Lines", "Differentiation", "Integration", "Trigonometry"],
    "Advanced Higher": ["Methods In Algebra", "Calculus", "Geometry", "Proof", "Complex Numbers"]
  },
  Physics: {
    icon: <Atom />,
    color: "purple",
    "National 5": ["Dynamics", "Space", "Electricity", "Waves", "Radiation"],
    Higher: ["Our Dynamic Universe", "Particles & Waves", "Electricity", "Researching Physics", "Astrophysics"],
    "Advanced Higher": ["Rotational Motion", "Waves", "Quantum Physics", "Electromagnetism", "Investigating Physics"]
  },
  Chemistry: {
    icon: <FlaskConical />,
    color: "orange",
    "National 5": ["Chemical Changes", "Structure & Bonding", "Nature's Chemistry", "Chemistry In Society", "Practical Skills"],
    Higher: ["Chemical Changes", "Structure", "Nature's Chemistry", "Researching Chemistry", "Equilibria"],
    "Advanced Higher": ["Inorganic Chemistry", "Physical Chemistry", "Organic Chemistry", "Instrumental Analysis", "Researching Chemistry"]
  }
};

const HOUSES = [
  { name: "NITH", points: 1250, color: "red" },
  { name: "SCAUR", points: 980, color: "blue" },
  { name: "CAIRN", points: 870, color: "green" }
];
...
function Auth({theme,setTheme}){
  ...
  async function submit(){
    ...
    else if(mode==="signup") setMsg("Account created. Check your email if confirmation is required.");
  }
  return (
    <main className="auth">
      <section className="auth-card">
        ...
        <p>Streaks, XP, gems, quests, topic maps and house battles for Scottish qualifications.</p>
...
function Game({user,theme,setTheme}){
  ...
  const [subject,setSubject]=useState(localStorage.getItem("rr_subject") || "Biology");
  const [level,setLevel]=useState(localStorage.getItem("rr_level") || "Higher");
  const [toast,setToast]=useState("");
  const [loading,setLoading]=useState(true);

  useEffect(()=>localStorage.setItem("rr_subject", subject),[subject]);
  useEffect(()=>localStorage.setItem("rr_level", level),[level]);
  ...
  async function load(){
    ...
    if(!p){
      ...
      const {data:newP,error}=await supabase.from("profiles").insert({
        id:user.id,email:user.email,username,display_name:username,house:"NITH"
      }).select("*").single();
      if(error) console.error(error);
      p=newP || {display_name:username,username,house:"NITH"};
    }
  ...
  const stats=useMemo(()=>{
    ...
    return {
      xp,
      levelNum:levelFromXP(xp),
      gems:Math.floor(xp/180),
      streak:calcStreak(sessions),
      total:sessions.reduce((a,s)=>a+Number(s.minutes||0),0),
      todayMinutes:todayS.reduce((a,s)=>a+Number(s.minutes||0),0),
      todayCount:todayS.length,
      week:week.reduce((a,s)=>a+Number(s.minutes||0),0)
    };
  },[sessions]);
    ...
    <div className="shell">
      <aside className="sidebar">
        ...
        <div className="player">
          ...
          <b>{profile?.display_name||"Student"}</b>
          <span>Level {stats.levelNum}</span>
          <div className="bar"><i style={{width:`${Math.min(100,(stats.xp%800)/8)}%`}}/></div>
          <small>{stats.xp%800}/800 XP</small>
          <div className="wallet"><span>🪙 {Math.floor(stats.xp/6)}</span><span>💎 {stats.gems}</span></div>
        </div>
      ...
      <main className="main">
        ...
        {page==="home" && <HomePage stats={stats} subject={subject} setSubject={setSubject} level={level} setLevel={setLevel} setPage={setPage}/>}
        {page==="study" && <Study user={user} subject={subject} setSubject={setSubject} level={level} setLevel={setLevel} reload={load} notify={notify}/>}
        ...
        {page==="shop" && <Shop/>}
        {page==="ai" && <AI stats={stats} sessions={sessions} profile={profile} subject={subject} level={level}/>}
...
function TopStats({stats,profile,theme,setTheme}){
  return <div className="top">
    <Metric icon="🔥" value={stats.streak} label="Day Streak"/>
    <Metric icon="⚡" value={stats.xp} label="XP"/>
    <Metric icon="💎" value={stats.gems} label="Gems"/>
    <div className="house-pill"><Shield size={18}/> {profile?.house||"NITH"}</div>
    <button className="circle"><Bell size={18}/></button>
    <button className="circle" onClick={()=>setTheme(theme==="light"?"dark":"light")}>{theme==="light"?<Moon size={18}/>:<Sun size={18}/>}</button>
    <Fox tiny/>
  </div>;
}
function Metric({icon,value,label}){ return <div className="metric"><b>{icon}</b><strong>{value}</strong><span>{label}</span></div>; }

function HomePage({stats,subject,setSubject,level,setLevel,setPage}){
  const current = COURSE_TOPICS[subject];
  return <div className="home-grid">
    <section className="hero">
      <div><h1>Keep your streak alive!</h1><p>{level} {subject}: study today and earn bonus XP.</p><button onClick={()=>setPage("study")}>Start Study Session</button></div>
      <Fox/>
    </section>
    <section className="quest-card">
      <h2>🎯 Daily Quest</h2><p>Complete 3 study sessions</p>
      <Progress value={Math.min(100,stats.todayCount/3*100)}/><b>{stats.todayCount} / 3</b>
      <div className="rewards"><span>💜 100 XP</span><span>🪙 50</span><span>🎁</span></div>
    </section>
    <section className="card selectors">
      <div><h2>Choose Subject</h2><SubjectPicker subject={subject} setSubject={setSubject}/></div>
      <div><h2>Choose Level</h2><LevelPicker level={level} setLevel={setLevel}/></div>
    </section>
    <section className="card map-card">
      <div className="head"><h2>{level} {subject} Progress</h2><button>View Topics ›</button></div>
      <LearningMap subject={subject} level={level} course={current}/>
    </section>
    <section className="card activity">
      <div className="head"><h2>📅 Today's Activity</h2><button>See All ›</button></div>
      <div className="activity-grid">
        <Mini icon="🕒" label="Study Time" value={minutesLabel(stats.todayMinutes)} goal="Goal: 3h 00m" pct={Math.min(100,stats.todayMinutes/180*100)}/>
        <Mini icon="🎯" label="Sessions" value={`${stats.todayCount} / 4`} goal="Daily Goal" pct={Math.min(100,stats.todayCount/4*100)}/>
        <Mini icon="⚡" label="XP Earned" value={stats.xp} goal="Daily Goal: 500 XP" pct={Math.min(100,(stats.xp%500)/5)}/>
      </div>
    </section>
...
function SubjectPicker({subject,setSubject}){
  return <div className="subject-grid">{Object.entries(COURSE_TOPICS).map(([name,s])=>
    <button key={name} className={`subject ${s.color} ${subject===name?"selected":""}`} onClick={()=>setSubject(name)}>
      {React.cloneElement(s.icon,{size:25})}<b>{name}</b>
    </button>)}</div>;
}
function LevelPicker({level,setLevel}){
  return <div className="level-grid">{LEVELS.map(l=>
    <button key={l} className={level===l?"level selected":"level"} onClick={()=>setLevel(l)}>{l}</button>)}</div>;
}
function Progress({value}){ return <div className="progress"><i style={{width:`${Math.max(0,Math.min(100,value))}%`}}/></div>; }

function LearningMap({subject,level,course}){
  const topics = course[level] || [];
  return <div className={`map ${course.color}`}>{topics.map((t,i)=>
    <div className={`node n${i+1} ${i>1&&i<4?"locked":""}`} key={t}>
      <div>{i>1&&i<4?<Lock size={28}/>:<Star size={28}/>}</div>
      <span>{i+1}</span><b>{t}</b><small>{i<2?"⭐⭐⭐":i===4?"🎁":"🔒"}</small>
    </div>)}</div>;
}
...
function Study({user,subject,setSubject,level,setLevel,reload,notify}){
  ...
  async function save(){
    const xp=minutes+(minutes>=60?50:0);
    const {error}=await supabase.from("study_sessions").insert({user_id:user.id,subject,minutes,xp,notes,check_in:`${level}: ${notes}`,session_date:today()});
    if(error){ notify(error.message); return; }
    notify(`Quest complete: ${minutes} min · ${xp} XP`);
    setRunning(false); setSeconds(0); setNotes(""); reload();
  }
  return <section className="page study">
    <h1>Study Session</h1>
    <SubjectPicker subject={subject} setSubject={setSubject}/>
    <LevelPicker level={level} setLevel={setLevel}/>
    <div className="timer"><Fox tiny/><b>{String(Math.floor(seconds/3600)).padStart(2,"0")}:{String(Math.floor(seconds/60)%60).padStart(2,"0")}:{String(seconds%60).padStart(2,"0")}</b><span>{level} {subject} quest in progress</span></div>
    <textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Write what you revised or one thing you learned..."/>
    <div className="actions">{!running?<button onClick={()=>setRunning(true)}><Play/>Start</button>:<button onClick={()=>setRunning(false)}>Pause</button>}<button className="green" onClick={save}><CheckCircle2/>End & Save</button></div>
  </section>;
}
...
function HouseCup({compact}){ return <section className={compact?"house compact":"page house"}><h2>🏆 House Cup</h2><p>Season ends in 12d 0h</p>{HOUSES.map(h=><div className={`house-row ${h.color}`} key={h.name}><Shield/><b>{h.name}</b><span>{h.points} pts</span></div>)}{!compact&&<p>Choose NITH, SCAUR or CAIRN in Settings.</p>}</section>; }
...
function AI({stats,sessions,profile,subject,level}){ const [q,setQ]=useState(""),[answer,setAnswer]=useState(""),[busy,setBusy]=useState(false); async function ask(prompt){ const question=prompt||q||`Make me a ${level} ${subject} revision plan.`; setBusy(true); try{ const res=await fetch("/api/ai-study-coach",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question,stats,recentSessions:sessions.slice(0,20),profile,subject,level})}); const data=await res.json(); setAnswer(data.answer||"Try again."); }catch{ setAnswer("AI could not respond right now."); } setBusy(false); } return <section className="page ai"><h1>AI Study Coach 🤖</h1><div className="quick"><button onClick={()=>ask(`Make me a ${level} ${subject} revision plan`)}>Revision Plan</button><button onClick={()=>ask("Which subject am I neglecting?")}>Weak Spots</button><button onClick={()=>ask("How do I keep my streak?")}>Streak Advice</button></div><div className="ai-chat"><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Ask anything about studying..." onKeyDown={e=>{if(e.key==="Enter")ask();}}/><button onClick={()=>ask()}><Send/>Ask</button></div><pre>{busy?"Thinking...":answer||"Ask your AI coach for revision tips."}</pre>{answer&&<button onClick={()=>copyText(answer)}><Copy/>Copy</button>}</section>; }
