...
import {
  Home, BookOpen, Trophy, Shield, Users, Store, Bot, BarChart3, Settings,
  Bell, Star, Leaf, Calculator, Atom, FlaskConical, LogOut, Sun, Moon, Send,
  Copy, CheckCircle2, Target, X, Search, UserRound, Palette, Timer, Play,
  Pause, Square, Brain, Award, Zap
} from "lucide-react";
import "./styles.css";
...
const SHOP = [
  ...
  { id:"coin_charm", name:"Coin Charm", icon:"🪙", costType:"coins", cost:40, desc:"Next reward earns double coins." },
  { id:"gem_finder", name:"Gem Finder", icon:"💎", costType:"coins", cost:75, desc:"Next reward gives 2 bonus gems." },
  { id:"streak_shield", name:"Streak Shield", icon:"🛡️", costType:"gems", cost:5, desc:"A profile badge for your streak." },
  { id:"fox_crown", name:"Fox Crown", icon:"👑", costType:"coins", cost:150, desc:"A royal character cosmetic." },
  { id:"house_banner", name:"House Banner", icon:"🏳️", costType:"coins", cost:100, desc:"Show off your house spirit." }
];
...
const TOPICS = {
  Biology: {
    ...
    "National 5":[
      ["Cell Biology","Cell structure, transport, DNA, enzymes, respiration and photosynthesis.","Draw cells, compare transport types, practise enzyme graphs and explain DNA to protein."],
      ["Multicellular Organisms","Specialised cells, tissues, organs, control, reproduction and inheritance.","Use flowcharts, revise control systems, practise genetics crosses and link structure to function."],
...
function makeQuiz(subject, level, topic){
  const base = TOPICS[subject][level].find(t=>t[0]===topic);
  const title = base?.[0] || topic;
  const summary = base?.[1] || "";
  const pool = [
    { q:`Which statement best matches ${title}?`, a:0, options:[summary, "It is only about spelling definitions.", "It is unrelated to the course.", "It is only needed after exams."] },
    { q:"What is the best first step when revising this topic?", a:1, options:["Skip examples", "Learn the key ideas and definitions", "Only read the title", "Avoid practice questions"] },
    { q:"What helps most with exam-style questions?", a:2, options:["Guessing quickly", "Not showing working", "Practising questions and checking feedback", "Ignoring mistakes"] },
    { q:"What should you do after getting an answer wrong?", a:0, options:["Find the mistake and redo a similar question", "Delete the score", "Never try the topic again", "Change subject immediately"] },
    { q:`For ${level}, what should your answers include?`, a:3, options:["One-word answers only", "No reasoning", "Random notes", "Clear steps, key terms and evidence"] },
    { q:"What is a strong study method?", a:1, options:["Only highlighting", "Active recall and practice", "Leaving it until the exam", "Studying with no target"] },
    { q:"How do you improve memory for this topic?", a:2, options:["Read once", "Avoid testing", "Space out revision and test yourself", "Never review"] },
    { q:"What should you track after a test?", a:0, options:["Score and weak areas", "Only the time", "Nothing", "Only your friend’s result"] },
    { q:"What makes a good summary?", a:3, options:["Copying a whole textbook", "No structure", "Only emojis", "Short key points in your own words"] },
    { q:"What is the goal of RevisionRoyale quizzes?", a:1, options:["Let users fake scores", "Mark answers and award XP fairly", "Avoid learning", "Stop streaks working"] },
  ];
  return pool.map((x,i)=>({...x, id:i+1}));
}
...
function Auth({theme,setTheme}){
  ...
  async function submit(){
    ...
    const result=mode==="login"
      ...
      : await supabase.auth.signUp({email,password,options:{data:{username:username||safeName(email),school,house,avatar:"🦊"}}});
  ...
  return <main className="auth-v6">
    <section className="welcome-side">
      <div className="floating">🦊</div>
      <h1>Study. Quiz. Level Up.</h1>
      <p>Join your school, complete real 10-question quizzes, earn XP, collect coins and climb your house leaderboard.</p>
      <div className="bubble-row"><span>🔥 Streaks</span><span>📝 Quizzes</span><span>⏱️ Study Timer</span><span>💎 Rewards</span></div>
    </section>
    <section className="auth-card-v6">
...
function Game({user,theme,setTheme}){
  ...
  async function ensureProfile(){
    ...
    if(!p){
      ...
      const{data:newP}=await supabase.from("profiles").insert({id:user.id,email:user.email,username,display_name:username,school:md.school||SCHOOLS[0],house:md.house||"NITH",avatar:md.avatar||"🦊",coins:0,gems:0,active_boost:null}).select("*").single();
      p=newP;
    }
    return p;
  }
  ...
  const notifications=[`You Have A ${stats.streak} Day Streak`,stats.activeBoost?`${boostName(stats.activeBoost)} Is Active`:"No Boost Active",`You Have ${stats.coins} Coins And ${stats.gems} Gems`];
...
function pageTitle(p){return({home:"Study Hub",quiz:"Topic Quiz",timer:"Study Timer",quests:"Daily Quests",leaderboard:"School Leaderboard",house:"House Cup",friends:"Friends",shop:"Boost Shop",character:"Customise Character",ai:"AI Study Coach",stats:"Progress Stats",settings:"Settings"}[p]||titleCase(p))}
...
function HomePage({stats,subject,setSubject,courseLevel,setCourseLevel,setPage}){
  return <div className="home-grid">
    <section className="hero"><div><h1>Study Smarter. Score Higher.</h1><p>Complete quizzes for score-based XP or use the timer for study-period XP.</p><div className="hero-actions"><button onClick={()=>setPage("quiz")}><Brain/> Take A Quiz</button><button className="alt" onClick={()=>setPage("timer")}><Timer/> Start Timer</button></div></div><Fox/></section>
    <section className="quest-card"><h2>🎯 Daily Quest</h2><p>Complete 2 Quizzes And 1 Study Timer Session</p><Progress value={Math.min(100,stats.todayCount/3*100)}/><b>{stats.todayCount} / 3</b><div className="rewards"><span>💜 XP</span><span>🪙 Coins</span><span>💎 Gems</span></div>{stats.activeBoost&&<p className="boost-active">Active Boost: {boostName(stats.activeBoost)}</p>}</section>
    ...
    <TopicMap subject={subject} courseLevel={courseLevel} setPage={setPage}/>
...
function TopicMap({subject,courseLevel,setPage}){const[open,setOpen]=useState(false),[selected,setSelected]=useState(null),course=TOPICS[subject],topics=course[courseLevel];return <section className="card map-card"><div className="head"><h2>{courseLevel} {subject} Topic Map</h2><button onClick={()=>setOpen(true)}>View Topics ›</button></div><div className="map-vibrant">{topics.map(([title,summary,revise],i)=><button className={`topic-node ${course.color}`} key={title} onClick={()=>setSelected({title,summary,revise,i})}><span>{i<2?<Star/>:<Award/>}</span><b>{title}</b><small>Tap To Learn</small></button>)}</div>{open&&<Modal title={`${courseLevel} ${subject} Topics`} close={()=>setOpen(false)}>{topics.map(([t,s,r],i)=><div className="topic-row clickable" key={t} onClick={()=>setSelected({title:t,summary:s,revise:r,i})}><b>{i+1}</b><div><strong>{t}</strong><p>{s}</p></div></div>)}</Modal>}{selected&&<Modal title={selected.title} close={()=>setSelected(null)}><p className="summary">{selected.summary}</p><div className="study-tips"><h3>Best Revision Method</h3><p>{selected.revise}</p><div className="modal-actions"><button onClick={()=>{setSelected(null);setPage("quiz")}}><Brain/> Take Quiz</button><button className="alt" onClick={()=>{setSelected(null);setPage("timer")}}><Timer/> Study Timer</button></div></div></Modal>}</section>}
...
function QuizPage({user,subject,setSubject,courseLevel,setCourseLevel,reload,notify,profile}){
  ...
  useEffect(()=>{setTopic(TOPICS[subject][courseLevel][0][0]);setAnswers({});setResult(null);setSaved(FalseFix())},[subject,courseLevel]);
  function FalseFix(){return false}
  const questions=useMemo(()=>makeQuiz(subject,courseLevel,topic),[subject,courseLevel,topic]);
  function mark(){
    const score=questions.reduce((a,q)=>a+(Number(answers[q.id])===q.a?1:0),0);
    setResult(score);setSaved(false);
  }
  async function save(){
    if(result===null){notify("Complete And Mark The Quiz First.");return}
    let xp=result*20, coins=Math.max(1,result*3), gems=result>=9?2:result>=7?1:0;
    if(profile?.active_boost==="double_xp")xp*=2;
    if(profile?.active_boost==="coin_charm")coins*=2;
    if(profile?.active_boost==="gem_finder")gems+=2;
    const {error}=await supabase.from("study_sessions").insert({user_id:user.id,subject,minutes:10,xp,score:result,notes:`Quiz: ${topic}`,check_in:`${courseLevel} · ${topic} · Quiz Score ${result}/10`,session_date:today(),session_type:"quiz"});
    ...
    setSaved(true);notify(`Quiz Saved: ${result}/10 · ${xp} XP · ${coins} Coins · ${gems} Gems`);reload();
  }
  return <section className="page quiz-page"><h1>10-Question Topic Quiz</h1><p className="summary">Students do not choose their score. The app marks the quiz and awards XP based on the result.</p><SubjectPicker subject={subject} setSubject={setSubject}/><LevelPicker courseLevel={courseLevel} setCourseLevel={setCourseLevel}/><label>Topic</label><select value={topic} onChange={e=>{setTopic(e.target.value);setAnswers({});setResult(null);setSaved(false)}}>{TOPICS[subject][courseLevel].map(([t])=><option key={t}>{t}</option>)}</select><div className="quiz-list">{questions.map(q=><div className="question" key={q.id}><h3>{q.id}. {q.q}</h3>{q.options.map((op,i)=><label className={`option ${Number(answers[q.id])===i?"picked":""}`} key={op}><input type="radio" name={`q${q.id}`} checked={Number(answers[q.id])===i} onChange={()=>setAnswers({...answers,[q.id]:i})}/>{op}</label>)}</div>)}</div><div className="quiz-actions"><button onClick={mark}><CheckCircle2/> Mark Quiz</button>{result!==null&&<button className="green" onClick={save} disabled={saved}><Zap/> {saved?"Saved":"Save And Earn XP"}</button>}</div>{result!==null&&<div className="result-card"><h2>Your Score: {result}/10</h2><p>{result>=9?"Excellent. Maximum rewards unlocked.":result>=7?"Strong result. Keep building.":result>=4?"Good attempt. Revise weak points and retry.":"Revise the topic summary, then try again."}</p><b>XP: {result*20}</b></div>}</section>
}
...
function StudyTimer({user,subject,setSubject,courseLevel,setCourseLevel,reload,notify,profile}){
  ...
  const projectedXP=Math.round(minutes*(200/60));
  async function save(){
    let xp=projectedXP, coins=Math.max(1,Math.floor(xp/15)), gems=minutes>=60?1:0;
    if(profile?.active_boost==="double_xp")xp*=2;
    if(profile?.active_boost==="coin_charm")coins*=2;
    if(profile?.active_boost==="gem_finder")gems+=2;
    const {error}=await supabase.from("study_sessions").insert({user_id:user.id,subject,minutes,xp,score:null,notes,check_in:`${courseLevel} · Study Timer · ${minutes} minutes`,session_date:today(),session_type:"timer"});
    if(error){notify(error.message);return}
    await supabase.from("profiles").update({coins:Number(profile?.coins||0)+coins,gems:Number(profile?.gems||0)+gems,active_boost:null}).eq("id",user.id);
    notify(`Study Saved: ${minutes} Min · ${xp} XP · ${coins} Coins · ${gems} Gems`);
...
function SchoolLeaderboard({profile}){const[rows,setRows]=useState([]);useEffect(()=>{if(!profile?.school)return;supabase.from("leaderboard_weekly").select("*").eq("school",profile.school).limit(50).then(({data})=>setRows(data||[]))},[profile?.school]);return <section className="page"><h1>{profile?.school||"School"} Leaderboard</h1>{rows.map((r,i)=><div className="leader" key={r.user_id}><span>#{i+1}</span><b>@{r.username}</b><em>{minutesLabel(r.minutes)}</em><strong>{r.xp} XP</strong></div>)}{!rows.length&&<p>No Leaderboard Data Yet. Save A Quiz Or Timer Session To Appear Here.</p>}</section>}
...
function Character({profile,setProfile,notify}){const[avatar,setAvatar]=useState(profile?.avatar||"🦊");async function save(){const{data,error}=await supabase.from("profiles").update({avatar}).eq("id",profile.id).select("*").single();if(error){notify(error.message);return}setProfile(data);notify("Character Saved.")}return <section className="page character"><h1>Customise Character</h1><div className="character-preview">{avatar}</div><div className="avatar-grid">{AVATARS.map(a=><button key={a} className={avatar===a?"avatar-choice selected":"avatar-choice"} onClick={()=>setAvatar(a)}>{a}</button>)}</div><button onClick={save}><Palette/>Save Character</button></section>}
